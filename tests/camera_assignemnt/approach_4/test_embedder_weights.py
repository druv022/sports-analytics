...
from src.camera_assignemnt.approach_4.embedder import (
...
def test_resolve_resnet50_weights_downloads_with_ssl_patch(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "src.camera_assignemnt.approach_4.embedder.download_file",
        fake_download,
    )
    with patch("src.camera_assignemnt.approach_4.embedder.torch") as mock_torch:
