...
from src.camera_assignemnt.scene_classifier.config import COURT_HSV_RANGES, Config
from src.camera_assignemnt.scene_classifier.models import Frame, SceneMlpError, SceneType
...
_VALID_SCENE_TYPES = frozenset({"full_court", "closeup"})
_MAX_SCALED_FEATURE_ABS = 1e3
...
def _histogram_usable(histogram: NDArray[np.float32]) -> bool:
    features = np.asarray(histogram, dtype=np.float64).reshape(-1)
    if features.size == 0 or not np.all(np.isfinite(features)):
        return False
    return float(features.sum()) > 0.0


def _scaled_features_usable(scaled: NDArray[np.float64]) -> bool:
    if scaled.size == 0 or not np.all(np.isfinite(scaled)):
        return False
    return float(np.max(np.abs(scaled))) <= _MAX_SCALED_FEATURE_ABS


def _probe_mlp_pipeline(pipeline: Pipeline) -> bool:
    """Return False when scaler output or MLP inference is numerically unstable."""
    scaler = pipeline.named_steps.get("scaler")
    mlp = pipeline.named_steps.get("mlp")
    ...
    probe = np.full((1, n_features), 1.0 / n_features, dtype=np.float64)
    scaled = scaler.transform(probe)
    if not _scaled_features_usable(scaled):
        return False
...
def _try_predict_scene_type(
    histogram: NDArray[np.float32],
    pipeline: Pipeline,
) -> SceneType | None:
    if not _histogram_usable(histogram):
        return None
    ...
    features = np.asarray(histogram, dtype=np.float64).reshape(1, -1)
    ...
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("error", RuntimeWarning)
            label = mlp.predict(scaled)[0]
    except RuntimeWarning:
        return None

    if label not in _VALID_SCENE_TYPES:
        return None
...
def predict_scene_type(
    ...
    label = _try_predict_scene_type(histogram, pipeline)
    if label is None:
        raise ValueError("MLP prediction failed due to invalid features or unstable model")
    return label
...
@lru_cache(maxsize=4)
def _scene_mlp_is_usable(model_path: str) -> bool:
    return _probe_mlp_pipeline(_load_cached_scene_mlp(model_path))


def resolve_scene_mlp(
    config: Config,
    project_root: Path | None = None,
) -> Pipeline:
    """Load the scene-type MLP or raise if it is missing or unusable."""
    model_path = config.resolved_scene_mlp_path(project_root)
    if not model_path.is_file():
        searched = config.scene_mlp_search_paths(project_root)
        searched_text = "\n".join(f"  - {path}" for path in searched)
        raise SceneMlpError(
            f"Scene MLP model not found at {model_path}. "
            "Train one with: python scripts/train_scene_classifier.py\n"
            f"Searched:\n{searched_text}"
        )

    resolved = str(model_path.resolve())
    if not _scene_mlp_is_usable(resolved):
        raise SceneMlpError(
            f"Scene MLP at {model_path} failed usability checks and cannot be used."
        )
    return _load_cached_scene_mlp(resolved)
...
def classify_scene(
    ...
    *,
    ...
    """Classify a scene with the trained MLP (no heuristic fallback)."""
    court_ratio, court_mask = compute_court_mask(frame, config)
    pipeline = resolve_scene_mlp(config, project_root=project_root)
    histogram = multi_region_hsv_histogram(frame, config)
    scene_type = _try_predict_scene_type(histogram, pipeline)
    if scene_type is None:
        raise SceneMlpError(
            "Scene MLP prediction failed due to invalid features or unstable inference."
        )
